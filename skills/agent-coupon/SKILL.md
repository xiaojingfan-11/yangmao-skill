...
2. After the city reply, call the offer search with that city, `limit: 5`, and no category filter. Follow each returned opaque cursor until it is null or 20 offers have been collected.
...
For each page, run the search bridge:

```bash
node "$CODEBUDDY_PLUGIN_ROOT/scripts/workbuddy-search.mjs" '{"city":"530100","limit":5}'
```

Use the city code supplied by the user/context; the example is not a default. On later pages include the returned cursor unchanged. Never inspect, construct, or modify a cursor.

Number the activities. Show exact title, summary, scope, validity when returned, and the returned first-party claim URL. Since offers with no city are nationwide, label them `全国可用`. Never describe nationwide offers as local merchant offers. State the number actually displayed, not the separate catalog/discovery count.
